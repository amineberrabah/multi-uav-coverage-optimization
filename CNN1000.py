...
# ════════════════════════════════════════════════
def draw_drone(ax, x, y, size=35):
    """Dessine un drone quadrirotor noir centré en (x, y). size adapté à l'échelle 1000."""
    arm   = size * 0.8
    r_mot = size * 0.35
    r_bod = size * 0.28
...
print(f"       zone={xmax}×{ymax}  n_cibles={n_cibles}  R={R}"
      f"  densité={densite_init:.6f}/u²  taux_attendu={taux_att_init:.2%}")
...
W_PRIORITY = 0.2
T_ASSIGN   = 3.0
SIGMA_SPRD = 200.0    # ×10 par rapport à l'échelle 100

_gy, _gx  = np.mgrid[0:GRID, 0:GRID]
_pix_scale = SCALE / GRID
PIX_X      = torch.tensor(_gx * _pix_scale + _pix_scale/2, dtype=torch.float32).flatten().to(DEVICE)
PIX_Y      = torch.tensor(_gy * _pix_scale + _pix_scale/2, dtype=torch.float32).flatten().to(DEVICE)
...
def deplacement(p_drone, p_dest):
    ...
    if dist < 50:          # seuil ×5 pour l'échelle 1000
        return p_dest
    return p_drone + (d / dist) * 5.0   # pas ×5
...
def barycentre_pmap_non_couverte(pos_drones):
    ...
    pix_scale = SCALE / GRID
    gx_coords = (_gx * pix_scale + pix_scale / 2) / SCALE
    gy_coords = (_gy * pix_scale + pix_scale / 2) / SCALE
...
def actor_loss_cnn(pred, cov_pred_detached):
    ...
    L_priority = F.mse_loss(pos, bary.detach()) / (SCALE**2)
...
def update(frame):
    ...
    centroides_cnn = pred_np.reshape(n_drones, 2) * SCALE
    ...
    convergence  = np.all(dist_to_cnn < 10)   # seuil ×5
    ...
    if convergence and cout_total <= COUT_SEUIL:
        ...
        positions_predites = pred_new[0].cpu().numpy().reshape(n_drones + 1, 2) * SCALE
    ...
    for pos in positions_drones:
        draw_drone(ax, pos[0], pos[1], size=35)
    ...
    for i, pos in enumerate(positions_drones):
        ax.text(pos[0], pos[1] + 15, f"T:{taux_drones[i]:.1f}%", fontsize=8, color='blue')
